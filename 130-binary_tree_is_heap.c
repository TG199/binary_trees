 ...
 */
int tree_is_complete(const binary_tree_t *tree, int idx, int num)
{
	if (tree == NULL)
		return (1);
	...
	return (tree_is_complete(tree->left, (2 * idx) + 1, num) &&
		tree_is_complete(tree->right, (2 * idx) + 2, num));
}
...
int binary_tree_is_complete(const binary_tree_t *tree)
{
	...
	return (tree_is_complete(tree, 0, num));
}
